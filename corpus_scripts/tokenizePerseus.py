...
errorLog = open('%s/tokenize_errors.txt'%logf,'w')
	...
	for elem in sourceText.iter():
		...
		if (tagWONS(elem.getparent())[:3] == 'div' or tagWONS(elem.getparent()) == 'sp' or tagWONS(elem.getparent()) == 'said' or tagWONS(elem.getparent()) == 'q') and (tagWONS(elem)=="p" or tagWONS(elem)=="l" or tagWONS(elem)=="quote" or tagWONS(elem)=="cit"):
			...
			for idx, snippet in enumerate(rightNodes):
				snippetAdd = False
				snippetQuote = False
				if (tagWONS(snippet.getparent()) == 'note' and snippet.is_text) or len(snippet.getparent().xpath('ancestor::note'))>0 or len(snippet.getparent().xpath('ancestor::n:note', namespaces={'n' : 'http://www.tei-c.org/ns/1.0'}))>0:
					snippet = ""
				elif (tagWONS(snippet.getparent()) == 'bibl' and snippet.is_text) or len(snippet.getparent().xpath('ancestor::bibl'))>0 or len(snippet.getparent().xpath('ancestor::n:bibl', namespaces={'n' : 'http://www.tei-c.org/ns/1.0'}))>0:
					snippet = ""
				elif (tagWONS(snippet.getparent()) == 'add' and snippet.is_text) or len(snippet.getparent().xpath('ancestor::add'))>0 or len(snippet.getparent().xpath('ancestor::n:add', namespaces={'n' : 'http://www.tei-c.org/ns/1.0'}))>0:
					snippetAdd = True
				elif (tagWONS(snippet.getparent()) == 'quote' and snippet.is_text) or len(snippet.getparent().xpath('ancestor::quote'))>0 or len(snippet.getparent().xpath('ancestor::n:quote', namespaces={'n' : 'http://www.tei-c.org/ns/1.0'}))>0:
					snippetQuote = True
				if snippetAdd == True:
					snippet = "2%s"%snippet
			...
			for words in currentText.split():
				...
				if words.startswith("(") and isunicode == True:
					...
					#create word node
					if wordForm != '' and wordForm != '"' and wordForm != "'":
