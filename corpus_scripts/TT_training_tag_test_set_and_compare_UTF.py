...
os.system("{tt_folder}/tree-tagger {curr_folder}/TreeTaggerData/ancient_greek_utf.dat {curr_folder}/TreeTaggerData/test_set_utf.txt {curr_folder}/TreeTaggerData/test_set_tagged_utf.txt -token".format(tt_folder=treetagger, curr_folder=os.path.dirname(os.path.realpath(__file__))))
	...
	if filename.find('benchmark')==-1 and filename.find('tagged')==-1 and filename[-3:]=='txt':
		...
		for record in files:
			if record[h['TLG Author']].value == author and record[h['TLG ID']].value == work: #retrieve metadata
				...
				break			
		#tag texts
		os.system("{tt_folder}/tree-tagger {curr_folder}/TreeTaggerData/ancient_greek_utf.dat {curr_folder}/TreeTaggerData/TestSetFilesUTF/{filename} {curr_folder}/TreeTaggerData/TestSetFilesUTF/{filename}_tagged.txt -token".format(tt_folder=treetagger, curr_folder=os.path.dirname(os.path.realpath(__file__)), filename=filename))
