...
os.system("{tt_folder}/tree-tagger {curr_folder}/TreeTaggerData/ancient_greek_utf.dat {curr_folder}/TreeTaggerData/test_set_utf.txt {curr_folder}/TreeTaggerData/test_set_tagged_utf.txt -token -lemma".format(tt_folder=treetagger, curr_folder=os.path.dirname(os.path.realpath(__file__))))

a_lines=[x for x in open('TreeTaggerData/test_set_tagged_utf.txt', 'r')]
c_lines=[x for x in open('TreeTaggerData/test_set_benchmark_utf.txt', 'r')]
open('TreeTaggerData/test_set_results_utf.txt', 'w')
output = open('TreeTaggerData/test_set_results_utf.txt', 'a')
trues = 0
...
for filename in os.listdir('TreeTaggerData/TestSetFilesUTF'):
	if filename.find('benchmark')==-1 and filename.find('tagged')==-1 and filename[-3:]=='txt':
		...
		for record in files:
			if record[h['TLG Author']].value == author and record[h['TLG ID']].value == work: #retrieve metadata
				...
				break			
		#tag texts
		os.system("{tt_folder}/tree-tagger {curr_folder}/TreeTaggerData/ancient_greek_utf.dat {curr_folder}/TreeTaggerData/TestSetFilesUTF/{filename} {curr_folder}/TreeTaggerData/TestSetFilesUTF/{filename}_tagged.txt -token -lemma".format(tt_folder=treetagger, curr_folder=os.path.dirname(os.path.realpath(__file__)), filename=filename))
		
		#compare tagged with benchmark
		a_lines=[x for x in open('TreeTaggerData/TestSetFilesUTF/%s_tagged.txt'%filename, 'r')]
		c_lines=[x for x in open('TreeTaggerData/TestSetFilesUTF/%s.txt'%filename.replace('.txt','_benchmark'), 'r')]
...
output.close()
os.system("clear && printf '\e[3J'")
print(open('TreeTaggerData/test_set_results_utf.txt','r').read())