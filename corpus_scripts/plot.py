...
def plot_stuff():
	...
	for file in input_files:
		...
		colours_p={'coral','turquoise','violet'}
		for line in lf:
			if re.search('[\-\d]', line[0]) == None:
				continue
			sense = line.split('\t')[-4]
			if sense =='wrong' or sense =='w':
				continue
			genre = line.split('\t')[1]
	...
	for genre,words in genres.items():
		...
		for word_idx,(word,senses) in enumerate(words.items()):
			...
			for sense_idx,(sense,centuries) in enumerate(senses.items()):
				...
				ax.bar(colPositions[word_idx],scores_plot, width,bottom=scores_plot_totals,color=colour_p[sense],label='%s: %s'%(word,word_senses[sense]))
	...
	for word,senses in words.items():
		...
		#plot area
		fig,ax=plot.subplots()
		ax.set_xlabel('Centuries', size='small')
		ax.set_xticks(Ncols)
		ax.set_xticklabels(colLabels, size='small')
		ax.set_title(word)
		ax.set_ylabel('Proportion (%) of sense per century', size='small')
		ax.set_ylim([0,107])
	...
	# 				labels[x][2] = ''
			...
			ax.bar(colPositions[sense_idx],scores_plot, width,color=colour_p[sense],label='%s: %s'%(word,word_senses[sense]))
	# 		for x in labels.values():
	# 			ax.text(x[0], x[1]+1, x[2], fontsize=8, horizontalalignment='center')
		...
		ax2=ax.twinx()
		ax2.set_ylabel('Proportion (%) of word per century (by genre)', size='small')
		ax2.set_ylim([0,107])
		...
		ax.legend(loc='upper left', bbox_to_anchor=(1.2, 1), shadow=False, ncol=1, fontsize='x-small')
		#ax.legend(loc='best', fontsize='x-small')
		#plot.tight_layout()
	...
	for word,senses in words.items():
		...
		#plot area
		fig,ax=plot.subplots()
		ax.set_xlabel('Genres', size='small')
		ax.set_xticks(Ncols)
		ax.set_xticklabels(colLabels,rotation='vertical')
		ax.set_title(word)
		ax.set_ylabel('Proportion (%) of sense per genre', size='small')
		ax.set_ylim([0,107])
		minor_ticks = numpy.arange(-6.5, 13.5, 1)
		ax.set_xticks(minor_ticks, minor=True)
		ax.grid(color='b', linestyle='dotted', linewidth=0.1, axis = 'x', which='minor')
