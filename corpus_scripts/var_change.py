...
def extract_data(gengr, sengr):
	...
	var_file = '%s/variation_data_G-%s_S-%s.txt'%(o_dir,gengr,sengr)
	ch_file = '%s/change_data_G-%s_S-%s.txt'%(o_dir,gengr,sengr)
	if os.path.isfile(ch_file): os.remove(ch_file)
	change_file = open(ch_file, 'a')
	if os.path.isfile(var_file): os.remove(var_file)
	variation_file = open(var_file, 'a')
...
###### VARIATION ######	
	...
	print('\n'.join(summary))
	variation_file.write('\n\n### SUMMARY ###\n\n')
	variation_file.write('Parameters: sense = %s, genres = %s\n\n'%(sengr,gengr))
	variation_file.write('\n'.join(sorted(summary)))
	del summary
...
extract_data('micro','micro')
extract_data('macro','micro')
extract_data('micro','macro')
extract_data('macro','macro')