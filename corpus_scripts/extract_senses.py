...
fields = ['date', 'genre', 'author', 'work', 'tlg author', 'tlg work', 'sentence location', 'sentence id', 'sentence ids', 'sentence original', 'target id', 'sense id', 'disambiguation','subgenre','subsense']
...
for idx,record in enumerate(files):
	...
	for tw,fname in tw_files.items():
		...
		for instance in allInstances:
			...
			subsense = curr_text.xpath('//sentence[@id="%s"]/word/lemma[@id="%s"]'%(sentence_id, tw))[0].get('sense')
			...
			instance = instance.replace('*\n', '*\t%s\t%s\t%s\t%s\t%s\t%s\n'%(form,tw,sense,notes,subgenre,subsense))
