...
type Model struct {
  ...
  Phi        map[int]*matrix.DenseMatrix
  Psi        map[int]*matrix.DenseMatrix
  N_k        map[int]*matrix.SparseMatrix //map[int][][]int
  N_sum_k    map[int]*matrix.SparseMatrix //map[int][][]int
...
func New_model(parameters *Model_parameters) (m *Model) {
  ...
  m.LogNormals_f    = make(map[int]*matrix.DenseMatrix, parameters.Num_categories)
  m.Phi             = make(map[int]*matrix.DenseMatrix, 1)
  m.Phi[0]          = matrix.Zeros(m.Parameters.Num_timestamps, m.Parameters.Num_categories)
...
func (m *Model) Initialize_batch(corpus *data.Corpus) {
  ...
  for tt:=0 ; tt<m.Parameters.Num_timestamps ; tt++ {
    for kk:=0 ; kk<m.Parameters.Num_categories ; kk++ {
      m.Phi[0].Set(tt, kk, (m.N_k[0].Get(tt,kk)+0.01) / (m.N_sum_k[0].Get(tt,0)+float64(m.Parameters.Num_categories)*0.01))
	...
	m.Psi[kk].Set(tt, vv, (m.N_k_f[kk].Get(tt,vv)+0.01) / (m.N_k_sum_f[kk].Get(tt,0)+float64(m.Parameters.Num_features)*0.01))
      ...
      m.LogNormals_f[kk].FillRow(tt, Inverse_alt(m.Psi[kk].RowCopy(tt)))
    }
    m.LogNormals_k[0].FillRow(tt, Inverse_alt(m.Phi[0].RowCopy(tt)))
  }
}
