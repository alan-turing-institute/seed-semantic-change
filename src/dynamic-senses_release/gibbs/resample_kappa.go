...
func Resample_kappa(old_kappa, a0, b0 float64, k, t int, phi map[int]*matrix.DenseMatrix) (new_kappa float64) {
  a,b      := Posterior_parameters(a0, b0, k, t, phi)
...
func Posterior_parameters(a0,b0 float64, k,t int, phi map[int]*matrix.DenseMatrix) (a,b float64) {
  ...
  for kk:=0 ; kk<k ; kk++ {
    ...
    mu_k := numstat.Mean(phi[0].ColCopy(kk), nil)

    for tt:=0 ; tt<t ; tt++ {
    b += math.Pow(phi[0].Get(tt,kk) - mu_k , 2)
    }
  }
  fmt.Println(b0, b)
  b = b0+ (b / 2.0)
