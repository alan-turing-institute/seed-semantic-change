...
func (s *Sampler) Estimate_Dirichlet(verbose bool, test_corpus *data.Corpus) (err error) {
  ...
  num_genres := s.Model.GetParameter("num_genres")


  pK             := make([][]float64, num_genres)
  pFgK          := make([][]float64, num_genres)
  posterior      := make([][]float64, num_genres)
  for g:=0 ; g<num_genres ; g++ {
    pK[g]             = make([]float64, num_categories)
    pFgK[g]          = make([]float64, num_categories)
    posterior[g]      = make([]float64, num_categories)
  }
...
func (s *Sampler) estimate_Dirichlet(pK [][]float64, pFgK [][]float64, posterior [][]float64, test_corpus *data.Corpus) (err error){
...
func (s *Sampler) Resample_categories_Dirichlet(pK [][]float64, pFgK [][]float64, posterior [][]float64) {
...
func (s *Sampler) resample_category_Dirichlet(doc *data.Document, pK [][]float64, pFgK [][]float64, posterior [][]float64) (new_k int) {
  ...
  new_k  = util.GetSampleLog_gen(s.generator, posterior[doc.Genre])
  fmt.Println(new_k)
