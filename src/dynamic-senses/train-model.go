    ...
    for wordIdx, word := range(words) {
        if len(word) > 0 {
            ...
            if _,err := os.Stat(path.Join(word_corpus_path, word, word_model_name)) ; err== nil {
                ...
                println("First case: using existing corpus")
            ...
            /* if no word-specific corpus exists -- create one! */
                ...
                println("Second case: creating corpus")
                println("full_heldout_path:", full_heldout_path)
                full_corpus := data.Load_corpus(path.Join(full_corpus_path))
                full_corpus.InitializeLabeling(num_top, 56)

                full_heldout := data.Load_corpus(path.Join(full_heldout_path))  //TODO: change into test set
            ...
            if len(word_corpus.Documents) >= min_doc_per_word && len(word_corpus.Documents) > 0 {
                ...
                //err = s.Estimate(false, &data.Corpus{}, mode) but want data.Corpus{} to be a different one! //VP TODO
