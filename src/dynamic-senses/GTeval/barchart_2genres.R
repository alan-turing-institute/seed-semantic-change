...
### This plots the output.dat from folder 59339 in the png file plotname for all the genres.
...
time_per <- #seq(time_val["start"], time_val["end"], by = time_val["dT"]) %>%
  unique(c(seq(time_val["start"], time_val["end"], by = time_val["dT"]),time_val["end"]+1)) %>%  #time_val["end"]+1 if times are not OK
...
data <- data.frame("Prob" = as.numeric(datatext[,1]), "Time" = as.numeric(datamat[, 1]), 
                   "K" = as.numeric(datamat[, 2]), "Time_periods" = rep(time_per, each = nlevK*2),
                   "K_bag" = rep(bag_words, times = nlevTime*2), "Genre" = rep(0:1, each=nlevK, times = nlevTime))
...
allplots <- plot_grid(plotlist = plotlist, ncol = 2, labels = 0:1, label_y = 1, label_x = 0.03, vjust = 0.95)
